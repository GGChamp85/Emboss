...
from .spec import (
    BulletList, Callout, CodeBlock, Heading, HorizontalRule,
    Image, MathBlock, NumberedList, PageBreak, Paragraph, Table, TextRun,
)
...
_CALLOUT_RE = re.compile(r"^>\s*\[!(NOTE|TIP|WARNING|CAUTION|IMPORTANT)\]\s*$", re.IGNORECASE)
...
def _parse_inline(text: str) -> list[TextRun]:
    ...
    patterns = [
        (_INLINE_BOLD_ITALIC, lambda m: TextRun(text=m.group(1) or m.group(2), bold=True, italic=True)),
...
def _collect_list_items(lines: list[str], start: int, pattern: re.Pattern,
                        base_indent: int = 0) -> tuple[list, int]:
    ...
    while i < len(lines):
        ...
        if not m:
            bm = _BULLET_RE.match(lines[i]) if pattern == _NUMBERED_RE else _NUMBERED_RE.match(lines[i])
            if bm and len(bm.group(1)) > base_indent:
                sub_pattern = _NUMBERED_RE if pattern == _BULLET_RE else _BULLET_RE
                sub_items, i = _collect_list_items(lines, i, sub_pattern, len(bm.group(1)))
                if items:
                    items[-1] = [items[-1]] + sub_items if isinstance(items[-1], str) else items[-1] + sub_items
                continue
            break
...
def parse_markdown(text: str) -> list:
    ...
    while i < len(lines):
        ...
        if cm:
            ...
            variant_map = {"note": "note", "tip": "tip", "warning": "warning",
        ...
        if im:
            alt_text = im.group(1)
            source = im.group(2)
            elements.append(Image(source=source, alt_text=alt_text, caption=alt_text or None))
            i += 1
            continue

        if line.strip().startswith("|") and i + 1 < len(lines) and _TABLE_SEP_RE.match(lines[i + 1].strip()):
            ...
            for row in rows:
                ...
                for ci, cell in enumerate(row):
                    ...
                    table_row.append(TableCell(content=cell, align=align if align != "left" else None))
                table_rows.append(table_row)
            header_cells = []
            for ci, h in enumerate(headers):
                align = aligns[ci] if ci < len(aligns) else "left"
                header_cells.append(TableCell(content=h, align=align if align != "left" else None))
        ...
        while i < len(lines) and lines[i].strip():
            next_line = lines[i]
            if (_HEADING_RE.match(next_line) or _HR_RE.match(next_line.strip()) or
                next_line.strip().startswith("```") or next_line.strip().startswith("|") or
                _BULLET_RE.match(next_line) or _NUMBERED_RE.match(next_line) or
                _IMAGE_RE.match(next_line.strip()) or _MATH_BLOCK_RE.match(next_line.strip()) or
                _CALLOUT_RE.match(next_line.strip()) or _PAGE_BREAK_RE.match(next_line.strip())):
                break
        ...
        if len(runs) == 1 and not runs[0].bold and not runs[0].italic and runs[0].font_family is None:
